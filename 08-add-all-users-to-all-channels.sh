...
sudo -u mattermost /opt/mattermost/bin/mmctl --local --strict channel list "$1" | grep -v 'off-topic\|town-sq\|^[}]' | sudo -u mattermost xargs -rd\\n dash -c 'us=$(/opt/mattermost/bin/mmctl --local --strict user list | awk "{if(/channelexport|github|doodle|pressup|simplepoll|slackimport|surveybot/){}else{print \$2}}") && for i in "$@" ; do /usr/bin/printf %q\  /opt/mattermost/bin/mmctl --local --strict channel users add "$1:$i" $us && echo ; done' dash "$1"
